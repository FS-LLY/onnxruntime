...
{
    Ort::Env env;
    std::string weightFile = "../VGG16_FashionMNIST_single_int8.onnx";
    ...
    static constexpr const int width_ = 32;  // 模型input width
    static constexpr const int height_ = 32; // 模型input height
    Ort::Value input_tensor_{nullptr};
    std::array<int64_t, 4> input_shape_{1, 1, height_, width_}; // NCHW, 1x3xHxW

    Ort::Value output_tensor_{nullptr};
    std::array<int64_t, 2> output_shape_{1, 10}; // 模型output shape，此处假设是二维的(1,10)

    std::array<float, width_ * height_ * 1> input_image_{}; // 输入图片，HWC
    std::array<float, 10> results_{};                       // 模型输出，注意和output_shape_对应
    std::string folderPath = "/dataset/FashionMNIST/test/32_32";
    int num = 0;
    int correct_num = 0;
    ...
    for (const auto &entry : fs::directory_iterator(folderPath))
    {
        ...
        if (std::regex_search(imgPath, match, labelRegex))
        {
            ...
            */
            ...
            int channels = 1;// img_f32.channels();
            ...
            for (int c = 0; c < channels; ++c)
            {
                for (int h = 0; h < height; ++h)
                {
                    ...
                    for (int w = 0; w < width; ++w)
                    {
                        ...
                        input_image_[index] = (img_data.at<cv::Vec3f>(h, w)[c]);
