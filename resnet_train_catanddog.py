...
class CatDogDataset(Dataset):
    def __init__(self, train=True):
        ...
        images = []
        labels = []
        pattern = re.compile(r'([a-zA-Z]+)\.(\d+)\.jpg')
        #images
        for root, _, files in os.walk("/dataset/cat_and_dog_train"):
            for name in files:
                #print(name)
                match = pattern.match(name)
                if match:
                    category = match.group(1)  # 提取种类
                    number = int(match.group(2))  # 提取编号并转换为整数
                    if number < 11500 and train:
                        images.append(os.path.join(root, name))
                        label = 1 if category == "cat" else 0
                        labels.append(label)
                    elif number >= 11500 and not train:
                        images.append(os.path.join(root, name))
                        label = 1 if category == "cat" else 0
                        labels.append(label)
...
def init_loader():
    trainset = CatDogDataset(train=True)
    testset = CatDogDataset(train=False)
    batch_size = 128
...
trainloader, testloader= init_loader()
...
torch.onnx.export(net,                                # model being run
                  ...
                  "resnet_catanddog.onnx",           # where to save the model (can be a file or file-like object)
